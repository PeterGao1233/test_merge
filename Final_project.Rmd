...
```{r}
p("The plot above shows how many athletes from each school in the top 30 athletes in a event. If a school has the most athltes in the top 30s, then that school perform the best in that event. For example, in 2024, there are around 7 athletes in the top 30s from Bethel, then Bethel performs the best in 100 meters." )
```
...
```{r}
p("In the plot above, we use the average place for each events to see how has they improved. So, in the running events, we can see that there is a trend line that shows the trend of the average places of the four class_year. For example, if the tredn line is upwarding, so the athletes of this class year is improved throughtout the years " )
```
...
```{r}
p("It seems like the wind does not influence the performance of athletes " )
```
