...
How does each class year preform?

```{r}
filtered_data <- all_running_events |>
  filter(year_date >= 2015 & year_date <= 2024)

avg_place <- filtered_data |>
  group_by(year_date) |>
  summarize(avg_place = mean(seed))

avg_class_year <- filtered_data %>%
  group_by(year = year(date)) %>%
  summarize(avg_class_year = mean(as.numeric(gsub("\\D", "", class_year))))


avg_place <- avg_place[-1, ]

```


```{r}
# Assuming you have a dataset named combined_table100 with columns 'class_year' and 'place'

# Group data by class year and calculate the mean pl
...
# Assuming you have a dataset named combined_table100 with columns 'class_year', 'place', and 'event'

# Group data by class year and event, count the number of participants who scored
...
# Create a bar plot
ggplot(class_count, aes(x = class_year, y = total_participants, fill = factor(class_year))) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  labs(title = "Total Participants by Class Year",
       x = "Class Year",
...
```{r}
renderPlot({
  ggplot(avg_place, aes(x = .data[[input$categ]])) +
