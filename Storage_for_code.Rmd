...
combined_tableHighJump$wind <- "NA"
combined_tableHighJump$event <- "High Jump"
```
...
combined_tablePoleVault$wind <- "NA"
combined_tablePoleVault$event <- "Pole Vault"
...
combined_tableLongJump <- combined_tableLongJump |> 
  rename(seed = Column_1, name = Column_2, class_year = Column_3, school = Column_4, mark = Column_5, wind = Column_6, meet = Column_7, date = Column_8)

print(combined_tableLongJump)
combined_tableLongJump$wind <- "NA"
combined_tableLongJump$event <- "Long Jump"
combined_tableLongJump$conv <- "NA"
```
...
combined_tableTripleJump$event <- "Triple Jump"
...
combined_tableShotPut$wind <- "NA"
combined_tableShotPut$event <- "Shot Put"
...
combined_tableDiscus$wind <- "NA"
combined_tableDiscus$event <- "Discus"
...
combined_tableHammer$wind <- "NA"
combined_tableHammer$event <- "Hammer"
...
combined_tableJavelin$wind <- "NA"
combined_tableJavelin$event <- "Javelin"
...
```{r, include = FALSE}
all_field_events <- rbind(combined_tableHighJump, combined_tablePoleVault, combined_tableLongJump, combined_tableTripleJump, combined_tableShotPut, combined_tableDiscus, combined_tableHammer, combined_tableJavelin)
```

```{r}
write_rds(all_field_events, "AllFieldEvents.csv")
```
...
```{r, include = FALSE}
all_relays <- rbind(combined_table4x100, combined_table4x400)
```

```{r}
write_rds(all_relays, "AllRelays.CSV")
```
...
```{r}
write_rds(combined_tableDecathlon, "Decathalon.CSV")
```
...
```{r, include = FALSE}
all_running_wind_events <- rbind(combined_table100, combined_table200, combined_table110H)
```

```{r}
write_rds(all_running_wind_events, "AllRunningWindEvents.csv")
```
...
```
```{r}
write_rds(combined_tableTripleJump, "AllfieldWindEvents.csv")
```
