...
```{r}
summary_table<- all_running_events %>%
    mutate(date = mdy(date)) %>%
    mutate(year_date = year(date))
view(summary_table)
```
...
all_running_events |>
  ...
  arrange(school, class_year, date) |>
    ...
    ggplot(aes(x = class_year, y = average_time)) +
    geom_bar() +
...
```{r, include = FALSE}
all_jumping_events <- rbind(combined_tableHighJump, combined_tablePoleVault, combined_tableLongJump, combined_tableTripleJump, combined_tableShotPut, combined_tableDiscus, combined_tableHammer, combined_tableJavelin)
```
